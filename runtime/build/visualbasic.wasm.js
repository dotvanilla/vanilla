    ...
    (function (JsArray) {
        ...
        */
        ...
        function toArray(array) {
            let a = WebAssembly.ObjectManager.getObject(array);
            let copy = [...a];
            return WebAssembly.ObjectManager.addObject(copy);
        }
        JsArray.toArray = toArray;
    ...
    */
    ...
    (function (JsString) {
        ...
        function substr(input, start, len) {
            let text = WebAssembly.ObjectManager.readText(input);
            let subtext = text.substr(start, len);
            return WebAssembly.ObjectManager.addText(subtext);
        }
        JsString.substr = substr;
        function split(input, delimiter) {
            let text = WebAssembly.ObjectManager.readText(input);
            let deli = WebAssembly.ObjectManager.readText(delimiter);
            let strings = text.split(deli);
            return WebAssembly.ObjectManager.addObject(strings);
        }
        JsString.split = split;
        function parseFloat(input) {
            let text = WebAssembly.ObjectManager.readText(input);
            return globalThis.parseFloat(text);
        }
        JsString.parseFloat = parseFloat;
        function parseInt(input) {
            let text = WebAssembly.ObjectManager.readText(input);
            return globalThis.parseInt(text);
        }
        JsString.parseInt = parseInt;
