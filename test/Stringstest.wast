...
(global $b (mut i32) (i32.const 100))
    ...
    (func $Main  (result i32)
        ...
        (local $str i32)
    (local $format i32)
    (set_local $str (call $i32_toString (call $string_add (call $string_add (call $Hello ) (i32.const 1)) (call $World ))))
    (set_local $format (call $i32_toString (call $string_add (call $string_add (call $string_add (call $string_add (call $string_add (call $string_add (call $string_add (i32.const 3) (call $i32_toString (get_global $a))) (i32.const 8)) (call $i32_toString (get_global $b))) (i32.const 12)) (call $f64_toString (get_global $C))) (i32.const 16)) (call $f64_toString (f64.add (f64.convert_s/i32 (get_global $a)) (f64.div (f64.convert_s/i32 (get_global $b)) (get_global $C)))))))
    ...
    (return (call $i32_toString (i32.const 20)))
    )
    (func $World  (result i32)
        ;; Public Function World() As string
        
    (return (call $i32_toString (i32.const 26)))
    )
    )